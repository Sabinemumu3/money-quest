...
(() => {
  ...
  const picnic = [
    {id:"bread",icon:"🍞",name:L("面包","Bread"),price:6,food:true},
    {id:"apple",icon:"🍎",name:L("苹果","Apple"),price:4,food:true},
    {id:"juice",icon:"🧃",name:L("果汁","Juice"),price:3},
    {id:"stickers",icon:"🌈",name:L("贴纸","Stickers"),price:5},
  ];
  const supplies = [
    {id:"pencil",icon:"✏️",name:L("铅笔","Pencil"),price:3},
    {id:"eraser",icon:"🧽",name:L("橡皮","Eraser"),price:2},
    {id:"stickers",icon:"🌈",name:L("贴纸","Stickers"),price:4},
    {id:"notebook",icon:"📒",name:L("小本子","Notebook"),price:2},
  ];
  ...
  function work() {
    if(state.stage===0) return `<h2>${t("野餐时，我最想带……","For the picnic, I'd love…")}</h2><div class="choice-stack">${choice(t("一盒果汁","A juice box"),"juice","wish",state.wish==="juice","🧃",t("甜甜的，野餐时喝","A sweet picnic drink"))}${choice(t("一张彩虹贴纸","Rainbow stickers"),"stickers","wish",state.wish==="stickers","🌈",t("给背包装饰一下","A little decoration for my bag"))}${choice(t("先看看，再决定","I'll look before deciding"),"none","wish",state.wish==="none","👀")}</div><div class="thinking-actions">${action(t("我来装背包 →","Let's pack →"),"start",!state.wish)}</div><p class="thinking-intro-note">${t("8–12 岁基础样板课 · 预留约 8–12 分钟，可随时休息。无需昵称、录音或付费。","Foundation sample for ages 8–12 · Allow about 8–12 minutes, with breaks whenever you like. No name, recording or payment required.")}</p>`;
    if(state.stage===1) return `<div class="wallet-strip"><span>${t("今天可以用","Today's budget")}</span><strong>10 🪙</strong></div><p class="goal-note">${t("任务：带一份吃的。水已准备好 💧","Task: pack some food. Water is already packed 💧")}</p>${shop(picnic,state.basket)}<p class="helper-copy">${t("每样最多一份。再点一次可以拿出来。","One of each at most. Tap again to remove it.")}</p><div class="thinking-actions">${action(t("先猜猜钱够不够 →","Make a prediction →"),"predict",!state.basket.length)}</div>`;
    if(state.stage===2) return `<h2>${t("还没付钱，先猜一下","Before paying, make a guess")}</h2><div class="backpack-strip">${selected(picnic,state.first).map(i=>`<span>${i.icon} ${h(tr(i.name))} ${i.price}</span>`).join("")}</div><div class="choice-stack" style="margin-top:16px">${Object.entries(predictions).map(([id,label])=>choice(tr(label),id,"prediction",state.prediction===id,{spare:"👛",exact:"🪙",short:"🤔",help:"🖐️"}[id])).join("")}</div><div class="thinking-actions">${action(t("试着付款，看一看 →","Try paying and see →"),"reveal",!state.prediction)}${action(t("我想改背包","Change my bag"),"repack",false,true)}</div>`;
